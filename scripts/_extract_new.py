...
# ═══════════════════════════════════════════════════════════════
# PDF 6: MILANO CERAMICA 2
# ═══════════════════════════════════════════════════════════════
def extract_mellano_ceramica2(pdf_path, output_dir):
    """40-page catalogue. Two page types:
    SAMPLE pages: big square tile (~44%) + 3 small variant squares + thin name bar at bottom.
    FULL pages: single large rect = room scene (application candidate).
    Strategy: extract thumbnail from each SAMPLE page, OCR product name from bottom,
    then use multi-patch histogram correlation to find best-matching FULL page as application.
    """
    print(f"\n{'='*60}")
    print(f"  MILANO CERAMICA 2")
    print(f"{'='*60}")
    ...
    def _multi_patch_corr(thumb, room):
        w, h = room.size
        floor = room.crop((w // 6, h // 2, w * 5 // 6, h))
        center = room.crop((w // 4, h // 4, w * 3 // 4, h * 3 // 4))
        return max(_hist_corr(thumb, floor), _hist_corr(thumb, center))
    ...
    # ── classify pages ───────────────────────────────────────
    sample_pages = []   # (page_index, thumbnail_img, product_name)
    full_pages = {}     # page_index -> room image

    for pn in range(4, n):  # skip first 4 intro pages
        lo = render_page(pdf_path, pn, dpi=100)
        wl, hl = lo.size
        arr = np.array(lo)
        rects = _find_large_rects(arr, min_area_ratio=0.03)
        rects = _remove_nested_rects(rects)

        # Check for big square (SAMPLE page indicator)
        big_sq = None
        for rx, ry, rw, rh in rects:
            area = (rw * rh) / (wl * hl)
            asp = rw / rh if rh else 0
            if 0.8 < asp < 1.2 and area > 0.25:
                if big_sq is None or area > big_sq[4]:
                    big_sq = (rx, ry, rw, rh, area)

        if big_sq and big_sq[4] < 0.50:
            # SAMPLE page - extract thumbnail at 200dpi
            hi = render_page(pdf_path, pn, dpi=200)
            sc = hi.size[0] / wl
            rx, ry, rw, rh, _ = big_sq
            crop = hi.crop((int(rx * sc), int(ry * sc),
                            int((rx + rw) * sc), int((ry + rh) * sc)))
            thumb = crop_to_content(crop)

            # OCR product name from bottom 10%
            w2, h2 = hi.size
            btm = hi.crop((0, int(h2 * 0.9), w2, h2))
            gray = btm.convert("L")
            gray = ImageEnhance.Contrast(gray).enhance(2.5)
            gray = gray.filter(ImageFilter.SHARPEN)
            raw = pyt.image_to_string(gray, config="--psm 7").strip()
            cleaned = re.sub(r"[^A-Za-z0-9\-\s]", "", raw).strip()
            # Extract product code pattern: WORD-DIGITS
            m = re.search(r"([A-Za-z]+[\-\s]\d{2,})", cleaned)
            name = m.group(1).replace(" ", "-").upper() if m else cleaned.split()[0] if cleaned else f"PRODUCT_P{pn + 1}"

            sample_pages.append((pn, thumb, name))
        else:
            # FULL page - room scene candidate
            hi = render_page(pdf_path, pn, dpi=200)
            full_pages[pn] = crop_to_content(hi)

    print(f"  Found {len(sample_pages)} products, {len(full_pages)} room pages")

    # ── match each product to best application ───────────────
    results = []
    for pn, thumb, name in sample_pages:
        # Score all full pages
        scores = []
        for fpn, room_img in full_pages.items():
            c = _multi_patch_corr(thumb, room_img)
            scores.append((c, fpn))
        scores.sort(reverse=True)

        # Prefer nearby (±5 pages) if decent
        nearby = [(c, fp) for c, fp in scores if abs(fp - pn) <= 5]
        if nearby and nearby[0][0] > 0.3:
            best_c, best_fp = nearby[0]
        else:
            best_c, best_fp = scores[0] if scores else (0.0, -1)

        app_img = full_pages.get(best_fp)
        tag = "[TA]" if thumb is not None and app_img is not None else "[T-]"
        print(f"  {name:24s} {tag}  app=P{best_fp + 1} ({best_c:.3f})")

        meta = _save_product(output_dir, "MILANO CERAMICA 2", name,
                             thumb, app_img, [pn + 1, best_fp + 1])
        meta["name"] = name
        results.append(meta)
...
if __name__ == "__main__":
    ...
    r6 = extract_mellano_ceramica2(
        "catalogues/MILANO CERAMICA 2.pdf", OUTPUT)
    
    print(f"\n\n{'='*60}")
    print(f"  SUMMARY")
    print(f"{'='*60}")
    for label, res in [("MELLANO2", r6)]:
        total = len(res)
